...
if initiator then
    ...
    if mainGame and mainGame:IsA("ModuleScript") then
        ...
        if type(mainGameModule.caption) == "function" then
            ...
            mainGameModule.caption("Hardcore-Remake Initiated", true)
        end
    end
end
...
local Config = {
    BulbModelID = "rbxassetid://12543866876",
    ChandelierKeywords = {"chandelier", "ceilinglight", "pendant", "hanglight"},
    JeffShopLightKeywords = {"stringlight", "christmaslight", "lightstring", "lightbulbstring"}
}

local processedRooms = {}
local darkRooms = {}
local darkRoomProcessed = {}

if Workspace:FindFirstChild("Ambience_Dark") then
    local oldPlayed = Workspace.Ambience_Dark.Played:Connect(function() end)
    oldPlayed:Disconnect()
    Workspace.Ambience_Dark.Played:Connect(function()
        local latestRoom = ReplicatedStorage.GameData.LatestRoom.Value
        local currentRoom = Workspace.CurrentRooms[tostring(latestRoom)]
        local basementRoom = Workspace:FindFirstChild("BasementRooms") and Workspace.BasementRooms[tostring(latestRoom)]
        local targetRoom = currentRoom or basementRoom
        if targetRoom and targetRoom:FindFirstChild("Assets") then
            local lightFolder = targetRoom.Assets:FindFirstChild("Light_Fixtures")
            if lightFolder and not darkRoomProcessed[targetRoom.Name] then
                for _, child in ipairs(lightFolder:GetDescendants()) do
                    if child:IsA("BasePart") then
                        pcall(function() child.CanCollide = false end)
                    end
                end
                darkRooms[targetRoom.Name] = true
                darkRoomProcessed[targetRoom.Name] = true
            end
        end
...
local function isChandelier(model)
    if not model or not model.Name then return false end
    local lowerName = model.Name:lower()
    for _, kw in ipairs(Config.JeffShopLightKeywords) do
        if lowerName:find(kw:lower()) then
            return true
        end
    end
    for _, kw in ipairs(Config.ChandelierKeywords) do
        if lowerName:find(kw:lower()) then
            return true
        end
    end
    if model:IsA("Model") then
        ...
        if primary and primary.Size.Magnitude > 8 then
            return true
        end
    end
    return false
...
local function HideBulbAndTurnOffLight(model)
    ...
    for _, child in ipairs(model:GetDescendants()) do
        if child:IsA("BasePart") then
            pcall(function() child.EmissiveStrength = 0 end)
        end
    end
end

local function GetUnprocessedWallLights()
    local unprocessedLights = {}
    local currentRooms = Workspace:FindFirstChild("CurrentRooms")
    if currentRooms then
        for _, room in ipairs(currentRooms:GetChildren()) do
            local roomId = room.Name
            if darkRooms[roomId] then
                if not darkRoomProcessed[roomId] then
                    local assets = room:FindFirstChild("Assets")
                    local lightFolder = assets and assets:FindFirstChild("Light_Fixtures")
                    if lightFolder then
                        for _, lightModel in ipairs(lightFolder:GetChildren()) do
                            if lightModel:IsA("Model") and not isChandelier(lightModel) then
                                table.insert(unprocessedLights, {
                                    light = lightModel,
                                    folder = lightFolder,
                                    roomId = roomId
                                })
    ...
    local basementRooms = Workspace:FindFirstChild("BasementRooms")
    if basementRooms then
        for _, room in ipairs(basementRooms:GetChildren()) do
            local roomId = "Basement_" .. room.Name
            if not processedRooms[roomId] then
                local assets = room:FindFirstChild("Assets")
                local lightFolder = assets and assets:FindFirstChild("Light_Fixtures")
                if lightFolder then
    ...
    return unprocessedLights
end

local function SpawnAllWallLights()
    local unprocessedLights = GetUnprocessedWallLights()
    if #unprocessedLights == 0 then return end
    for _, item in ipairs(unprocessedLights) do
        local oldLight = item.light
        local lightFolder = item.folder
        local bulbModel = nil
        local success = pcall(function()
            bulbModel = game:GetObjects(Config.BulbModelID)[1]
        end)
        if not success or not bulbModel then
            continue
        end
        HideBulbAndTurnOffLight(bulbModel)
        local spawnCFrame = oldLight.PrimaryPart and oldLight.PrimaryPart.CFrame or oldLight:GetPivot()
        bulbModel:PivotTo(spawnCFrame)
        bulbModel.Parent = lightFolder
        for _, child in ipairs(oldLight:GetDescendants()) do
            if child:IsA("BasePart") then
                pcall(function() child.CanCollide = false end)
            end
        end
...
ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    ...
    if currentRoom and currentRoom:FindFirstChild("Assets") then
        local lightFolder = currentRoom.Assets:FindFirstChild("Light_Fixtures")
        if lightFolder then
            for _, child in ipairs(lightFolder:GetDescendants()) do
                if child:IsA("BasePart") then
                    pcall(function() child.CanCollide = false end)
    ...
    local vu7 = game:GetObjects("rbxassetid://14123633468")[1]
    ...
    local vu20 = v15("https://github.com/Acupofsauce/document/raw/563891a2879b2247cdb72c567260b953b6f7667a/Rebound%E7%BB%8F%E8%BF%87%E5%89%8D%E9%9F%B3%E6%95%88.mp3", "ReboundMoving")
...
loadstring(game:HttpGet("https://glot.io/snippets/hemhlk1kto/raw/Doors-Hardcore-Mod-StaminaBar-Script.lua"))()
    ...
    if game.ReplicatedStorage.GameData.LatestRoom.Value <= 1 then
        ...
        spawn(function()
    local z9 = {}
    local function q7(w2, e5)
        local r6 = w2
        local t4 = Instance.new("Sound")
        local y1, u0 = pcall(function()
            if not isfile(e5 .. ".mp3") then
                writefile(e5 .. ".mp3", game:HttpGet(r6))
            end
            t4.SoundId = (getcustomasset or getsynasset)(e5 .. ".mp3")
        end)
        if not y1 then
            t4.SoundId = "rbxassetid://183890752"
        end
        return t4
    end
    while true do
        ...
        local function f6()
            local d1 = false
            local l5 = tick()
            local b4 = game:GetObjects("rbxassetid://12805509624")[1]
            b4.Parent = workspace
            b4.Name = "Deer God"
            local g9 = workspace.CurrentRooms:FindFirstChild("1")
            if g9 then
                b4:PivotTo(g9:GetPivot() + Vector3.new(0, 3, 0))
            else
                b4:PivotTo(CFrame.new(0, 3, -100))
            end
            local h3 = b4.PrimaryPart or b4:FindFirstChildWhichIsA("BasePart") or b4:FindFirstChild("HumanoidRootPart") or b4:FindFirstChild("Root")
            if not h3 then return end
            h3.Anchored = true
            h3.CanCollide = false
            local j0 = q7("https://raw.githubusercontent.com/Acupofsauce/document/c078e4043f7a52dbba8d3093ff6481871508f249/%E9%87%8D%E7%BD%AE%E7%89%88%E9%B9%BF%E7%A5%9E%E9%9F%B3%E6%95%88.mp3", "DeerGodSound")
            j0.Parent = game:GetService("SoundService")
            j0.Volume = 4
            j0.Looped = true
            j0.RollOffMinDistance = math.huge
            j0.RollOffMaxDistance = math.huge
            j0:Play()
            local n6 = Instance.new("Sound")
            n6.SoundId = "rbxassetid://6111244462"
            n6.Parent = b4
            n6.Volume = 1
            n6.Looped = true
            n6:Pause()
            local c8
            c8 = game:GetService("RunService").Heartbeat:Connect(function()
                if not b4 or not h3 then
                    c8:Disconnect()
                    n6:Stop()
                    return
                end
                local a5 = p2.Character
                local i4 = a5 and a5:FindFirstChild("HumanoidRootPart")
                if not i4 or not a5:FindFirstChild("Humanoid") or a5.Humanoid.Health <= 0 then
                    c8:Disconnect()
                    n6:Stop()
                    return
                end
                local v1 = (h3.Position - i4.Position).Magnitude
                if v1 <= 100 and not n6.IsPlaying then
                    n6:Play()
                elseif v1 > 100 and n6.IsPlaying then
                    n6:Pause()
                end
            end)
            spawn(function()
                while b4 and h3 and not d1 do
                    task.wait(0.05)
                    local a5 = p2.Character
                    local i4 = a5 and a5:FindFirstChild("HumanoidRootPart")
                    local o2 = a5 and a5:FindFirstChild("Humanoid")
                    if i4 and o2 then
                        local v1 = (h3.Position - i4.Position).Magnitude
                        if v1 <= k8 then
                            o2.Health = 0
                            d1 = true
                            DeathHint({
"You died from Deer God",
"He doesn't seem to be from this hotel, but he somehow got here."
}, "Blue")
game:GetService("ReplicatedStorage").GameStats["Player_"..game.Players.LocalPlayer.Name].Total.DeathCause.Value = "Deer God"
            ...
            -- 自动开门 穿门逻辑（直接加在这里）
            local currentRoomVal = game.ReplicatedStorage.GameData.LatestRoom.Value
            if workspace.CurrentRooms:FindFirstChild(tostring(currentRoomVal)) then
                local currentRoom = workspace.CurrentRooms[tostring(currentRoomVal)]
            ...
            while b4 and h3 and not d1 do
                ...
                -- 追玩家逻辑（原逻辑不动）
                local x2 = Vector3.new(i4.Position.X, h3.Position.Y, i4.Position.Z)
                local z4 = (x2 - h3.Position).Unit
                local w5 = s3 * 0.1
                local v1 = (x2 - h3.Position).Magnitude
                if v1 > w5 then
                    h3.CFrame = CFrame.new(h3.Position + z4 * w5)
                else
                    h3.CFrame = CFrame.new(x2)
                end
                task.wait(0.1)
            end
            if c8 then c8:Disconnect() end
            if j0 and j0.Parent then
                j0:Stop()
                j0:Destroy()
            end
            if n6 and n6.Parent then
                n6:Stop()
                n6:Destroy()
            end
            local r7 = q7("https://github.com/Acupofsauce/Music/raw/e9b3a4378d92ff712eb6819b495e1bee2c6fb6a8/%E9%B9%BF%E7%A5%9E%E6%B6%88%E5%A4%B1%E9%9F%B3%E6%95%88.mp3", "DeerGodDespawn1")
            r7.Parent = workspace
            r7.Volume = 1
            r7.Name = "DeerGodDespawnSound1"
            table.insert(z9, r7)
            r7:Play()
            local t8 = Instance.new("Sound")
            t8.SoundId = "rbxassetid://8258601891"
            t8.Parent = workspace
            t8.Volume = 5
            t8.Name = "DeerGodDespawnSound2"
            table.insert(z9, t8)
            t8:Play()
            t8.Ended:Connect(function()
                task.delay(0.5, function()
                    for i = #z9, 1, -1 do
                        local s9 = z9[i]
                        if s9 == r7 or s9 == t8 then
                            table.remove(z9, i)
            ...
            game.Debris:AddItem(b4, 5)
        end
        game:GetService("RunService").Heartbeat:Connect(function()
            for _, s9 in ipairs(z9) do
    ...
    local function vu494()
        while true do
            ...
            entity:SetCallback("OnMove", function()
                ...
                local root = silence:FindFirstChild("HumanoidRootPart") or silence:FindFirstChild("Root") or silence.PrimaryPart
                if not root then return end
                local rooms = workspace.CurrentRooms
                for i = lastRoom, game.ReplicatedStorage.GameData.LatestRoom.Value do
                    local room = rooms:FindFirstChild(tostring(i))
                    if room then
                        local target = room:FindFirstChild("RoomStart") or room:FindFirstChild("RoomEntrance")
                        if target and (root.Position - target.Position).Magnitude < 5 then
                            despawned = true
                            silence:Destroy()
                            break
                        end
            ...
            entity:SetCallback("OnDespawned", function()
                if despawned then return end
                despawned = true
                local function ShowSilenceAchievement()
                    if getgenv().GotSilenceAchievement then return end
                    getgenv().GotSilenceAchievement = true
                    task.spawn(function()
                        local Players = game:GetService("Players")
                        ...
                        if not globalUI then return end
                        local achievementsHolder = globalUI:FindFirstChild("AchievementsHolder")
                        if not achievementsHolder then return end
                        local achievementTemplate = achievementsHolder:FindFirstChild("Achievement")
                        if not achievementTemplate then return end
                        local frame = achievementTemplate:Clone()
                        frame.Name = "SilenceAchievement"
                        frame.Frame.Details.Title.Text = "Careful Listener"
                        frame.Frame.Details.Desc.Text = "Shhh.. do you hear that?"
                        if frame.Frame.Details:FindFirstChild("Reason") then
                            frame.Frame.Details.Reason.Visible = false
                        end
                        frame.Frame.ImageLabel.Image = "rbxassetid://101950034142799"
                        frame.Frame.TextLabel.Text = "UNLOCKED ACHIEVEMENT"
                        frame.Size = UDim2.new(0, 0, 0, 0)
                        frame.Frame.Position = UDim2.new(1.1, 0, 0, 0)
                        frame.Visible = true
                        frame.Parent = achievementsHolder
                        if frame:FindFirstChild("Sound") then
                            frame.Sound:Play()
                        end
                        frame:TweenSize(UDim2.new(1, 0, 0.2, 0), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.8, true)
                        frame.Frame:TweenPosition(UDim2.new(0, 0, 0, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.5, true)
                        ...
                        task.wait(6)
                        frame.Frame:TweenPosition(UDim2.new(1.1, 0, 0, 0), Enum.EasingDirection.In, Enum.EasingStyle.Quad, 0.5, true)
                        task.wait(0.5)
                        frame:TweenSize(UDim2.new(1, 0, -0.1, 0), Enum.EasingDirection.InOut, Enum.EasingStyle.Quad, 0.5, true)
                        task.wait(0.5)
                        frame:Destroy()
                    end)
                end
                ShowSilenceAchievement()
            end)
    ...
    vu494()
        ...
        spawn(function()
            ...
            local function vu293()
                while wait(850) do
                    local currentRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
                    if workspace:FindFirstChild("SeekMovingNewClone") or currentRoom == 3 or currentRoom == 49  then
                        vu293()
                        return
                    end
...
if not getgenv().GotNostalgicFrightAchievement then
    getgenv().GotNostalgicFrightAchievement = true
    Achievement({
        Title = "A Nostalgic Fright",
        Desc = "So muny familiar faces!",
        Reason = "",
        Image = "rbxassetid://136436172368777",
...
local entity = spawner.Create({
	...
	Death = {
		Type = "A-60",
		Hints = {"You encountered A60.", "He is very fast, so you need to hide quickly."},
		Cause = "A-60"
	}
})
entity:Run()
...
entity:SetCallback("OnDespawned", function()
    print("Entity has despawned, giving a Achievement.")
local function ShowA60Achievement()
    task.spawn(function()
        local Players = game:GetService("Players")
        local LocalPlayer = Players.LocalPlayer
        local TS = game:GetService("TweenService")
        local achievementsHolder = LocalPlayer.PlayerGui.GlobalUI.AchievementsHolder
        local achievementTemplate = achievementsHolder:FindFirstChild("Achievement")
        local frame = achievementTemplate:Clone()
        
        frame.Name = "A60Achievement"
        frame.Frame.Details.Title.Text = "The Unstoppable A60"
        frame.Frame.Details.Desc.Text = "Fate has no mercy"
        frame.Frame.Details.Reason.Text = "Survive the terrifying A60 and its endless pursuit"
        frame.Frame.ImageLabel.Image = "rbxassetid://12350986086"
        frame.Frame.TextLabel.Text = "UNLOCKED ACHIEVEMENT"
...
if not getgenv().GotA60Achievement then
    getgenv().GotA60Achievement = true
    ShowA60Achievement()
end

game:GetService("UserInputService").InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.K and not getgenv().GotA60Achievement then
        getgenv().GotA60Achievement = true
        ShowA60Achievement()
    end
end)
...
local entity = spawner.Create({
	...
	Death = {
		Type = "Curious",
		Hints = {"You met A-120", "He might be fast"},
		Cause = "A-120"
	}
})
entity:Run()
...
local function ShowSilenceAchievement()
    ...
    task.spawn(function()
        ...
        frame.Name = "SilenceAchievement"
        frame.Frame.Details.Title.Text = "Blistering Pursuit"
        frame.Frame.Details.Desc.Text = "Speed beyond reckoning"
        frame.Frame.Details.Reason.Text = "Survive the unrelenting charge of A-120"
        frame.Frame.ImageLabel.Image = "rbxassetid://12351008553"
        frame.Frame.TextLabel.Text = "UNLOCKED ACHIEVEMENT"
    ...
    function v576.OnIncomingMessage(p579, _)
        ...
        if p579.TextSource then
            ...
            if p579.TextSource.UserId == 8869098656 or p579.TextSource.UserId == 4322541912 or p579.TextSource.UserId == 3338249600 or p579.TextSource.UserId == 530829101 or p579.TextSource.UserId == 4332501203 or p579.TextSource.UserId == 1520423590 or p579.TextSource.UserId == 2763394267 or p579.TextSource.UserId == 3659299888 or p579.TextSource.UserId == 3962633044 or p579.TextSource.UserId == 9677119350 or p579.TextSource.UserId == 99999999 then
                ...
                if msg == "/like" or msg == "/entities" then
                    coroutine.wrap(function()
                        local entityList = {
                            "/60 - 召唤A-60",
                            "/90 - 召唤A-90",
