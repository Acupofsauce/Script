local restrictedIDs = {9541639131, 9667870106}

for _, p in ipairs(game.Players:GetPlayers()) do
    for _, id in ipairs(restrictedIDs) do
        if p.UserId == id then
            p:Kick("This player ID is on the blacklist")
            print("[Auth] Removed user: " .. p.Name .. " (ID: " .. id .. ")")
            break
        end
    end
end
...
game.Players.PlayerAdded:Connect(function(p)
    for _, id in ipairs(restrictedIDs) do
        if p.UserId == id then
            p:Kick("This player ID is on the blacklist")
            print("[Auth] Removed new user: " .. p.Name .. " (ID: " .. id .. ")")
            break
        end
    end
end)
        ...
        while true do
            ...
            entity:SetCallback("OnSpawned", function()
                ...
                deerGod.Destroying:Connect(function()
                    ...
                    local despawnSound1 = GetGitSound("https://github.com/Acupofsauce/Music/raw/e9b3a4378d92ff712eb6819b495e1bee2c6fb6a8/%E9%B9%BF%E7%A5%9E%E6%B6%88%E5%A4%B1%E9%9F%B3%E6%95%88.mp3", "DeerGodDespawn1")
                    despawnSound1.Parent = workspace
                    despawnSound1.Volume = 1
                    despawnSound1.Name = "DeerGodDespawnSound1"
                    table.insert(soundWhitelist, despawnSound1)
                    despawnSound1:Play()

                    despawnSound1.Ended:Connect(function()
                        local despawnSound2 = Instance.new("Sound")
                        despawnSound2.SoundId = "rbxassetid://8258601891"
                        despawnSound2.Parent = workspace
                        despawnSound2.Volume = 5
                        despawnSound2.Name = "DeerGodDespawnSound2"
                        table.insert(soundWhitelist, despawnSound2)
                        despawnSound2:Play()

                        despawnSound2.Ended:Connect(function()
                            task.delay(0.5, function()
                                for i = #soundWhitelist, 1, -1 do
                                    local snd = soundWhitelist[i]
                                    if snd == despawnSound1 or snd == despawnSound2 then
            ...
            entity:SetCallback("OnDespawned", function()
                if despawned then return end
                despawned = true
                if getgenv().GotDeerGodAchievement then return end
                getgenv().GotDeerGodAchievement = true
                ...
                local function ShowAchievement()
                    task.spawn(function()
                        local achievementsHolder = LocalPlayer.PlayerGui:FindFirstChild("GlobalUI"):FindFirstChild("AchievementsHolder")
                        local achievementTemplate = achievementsHolder:FindFirstChild("Achievement")
                        local frame = achievementTemplate:Clone()

                        frame.Name = "DeerGodAchievement"
                        frame.Frame.Details.Title.Text = "Last Chance to Look Away"
                        frame.Frame.Details.Desc.Text = "Why are you running?"
                        frame.Frame.Details.Reason.Text = "Survive the rare Entity called Deer God."
                        frame.Frame.ImageLabel.Image = "rbxassetid://88191964403924"
